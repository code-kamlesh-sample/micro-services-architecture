...
namespace Microservices.Web.Service
{
    public class CartService : ICartService
    {
        ...
        public async Task<ResponseDto?> ApplyCouponAsync(CardDto cartDto)
        {
        ...
        public async Task<ResponseDto?> UpsertCartAsync(CardDto cartDto)
        {
            return await _baseService.SendAsync(new RequestDto()
            {
                ApiType = ApiType.POST,
                Data = cartDto,
                Url = ShoppingCartAPIBase + "/api/cart/CardUpsert"
            });
        }
    }
}
